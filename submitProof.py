...
def merkle_assignment():
    ...
    random_leaf_index = 900 #TODO generate a random index from primes to claim (0 is already claimed)
...
def build_merkle(leaves):
    ...
    while len(tree[-1]) > 1:
        ...
        # Hash pairs and build the next layer
        for i in range(0, len(current_layer), 2):
            combined_hash = hash_pair(current_layer[i], current_layer[i + 1])
